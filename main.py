...
from data_utils import get_split_cifar100_tasks
...
parser.add_argument(
        '--experiment', type=str,
        choices=['core50','toybox','ilab','permutated-mnist', 'svhn-mnist', 'mnist-svhn'],
        default='core50'
    )
...
parser.add_argument('--generator-iterations', type=int, default=30)
...
if __name__ == '__main__':
    ...
    dataset = experiment
    returning = get_split_cifar100_tasks(num_tasks, args.batch_size,run,paradigm,dataset)
    train_datasets = returning[0]
    test_datasets = returning[1]
    #print("dataset recieved")
        #TrainDataLoaders,TestDataLoaders
    '''
    ...
    '''
    DATASET_CONFIGS = {'core50': {'size': 32, 'channels': 3, 'classes': 10},
                        'toybox': {'size': 32, 'channels': 3, 'classes': 12},
                        'ilab': {'size': 32, 'channels': 3, 'classes': 14}
    }

    if args.experiment == 'core50':
        dataset_config = DATASET_CONFIGS['core50']
    elif args.experiment == 'toybox':
        dataset_config = DATASET_CONFIGS['toybox']
    elif args.experiment == 'ilab':
        dataset_config = DATASET_CONFIGS['ilab']
        # define the models.
    cnn = CNN(
    ...
    #print("going inside scholar")
    scholar = Scholar(label, generator=wgan, solver=cnn)
    #print("outside scholar")
