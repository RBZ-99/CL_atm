...
def train(scholar, train_datasets, test_datasets, replay_mode,
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        #print("in task,train_dataset being enumarated from train_dataset ")
        ...
        #print("training the schoalrs with gen rplay")
...
def _solver_training_callback(
    ...
    def cb(solver, progress, batch_index, result):
        ...
        if iteration % eval_log_interval == 0:
            ...
            test_custom_data = [[] for i in range(current_task)]
            for i in range(current_task):
                print("in train i")
                print(i)
                test_custom_data[i] = test_datasets[i]
            precs = [
                utils.validate(
                    solver, test_custom_data,current_task, test_size=test_size,
